<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%;margin-top:15px;" border size="mini" v-loading="loading" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="typeName"  label="分类" align="center" width="70">  
      </el-table-column>
      <el-table-column label="状态" align="center" width="110">
        <template slot-scope="props">
          ...
          <el-button type="success" style="margin-left:0; padding: 7px 0;" size="mini" plain class="caozuoBtn" v-if="tuijianShow"
          ...
          <el-button type="success" style="margin-left:0" size="mini" class="caozuoBtn" v-if="noshenheShow"
                     @click="ArticleClass(props.row)">分类
          </el-button>
        </template>
      </el-table-column>
    ...
    <!-- 查询分类弹窗 -->
    <el-dialog title="查询分类" :visible.sync="centerDialogVisibleClass" width="30%" center>
      <!-- 一级分类 -->
      <el-select v-model="FiveValue" placeholder="请选择" @change="handleChange5">
        <el-option
      ...
      <!-- 二级分类 -->
      <el-select v-model="SixValue" placeholder="请选择" @change="handleChange6">
        <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisibleClass = false">取 消</el-button>
        <el-button type="primary" @click="checkClassItem">确 定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="分类" :visible.sync="classificationShow" width="30%" center>
      ...
      <!-- 图片的显示 -->
      <div class="imgBox">
        <div class="img-box" v-if="existingPicture">
          <img :src="$oss.url + existingPicture" alt="" width="150">
        </div>
        <ul class="imglist" v-else>
          <li v-for="(item, index) in imgList" :key="index">
            <img :src="$oss.url + item.picture" alt="" width="150" height="75" @click="chooseImg(index, item.picture)">
    ...
    <!-- 推荐文章分类 -->
    <el-dialog title="分类" :visible.sync="ArticleShow" width="30%" center>
      <!-- 一级分类 -->
      <el-select v-model="FirstArticleValue" placeholder="请选择" @change="handleChange3">
        <el-option
      ...
      <!-- 二级分类 -->
      <el-select v-model="SecondArticleValue" placeholder="请选择" @change="handleChange4">
        <el-option
      ...
      <!-- 图片的显示 -->
      <div class="imgBox">
        <div class="img-box" v-if="existingArticlePicture">
          <img :src="$oss.url + existingArticlePicture" alt="" width="150">
        </div>
        <ul class="imglist" v-else>
          <li v-for="(item, index) in imgList" :key="index">
            <img :src="$oss.url + item.picture" alt="" width="150" height="75" @click="chooseImg(index, item.picture)">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="ArticleDetermine">确 定</el-button>
      </span>
    </el-dialog>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ArticleShow: false,
        centerDialogVisibleClass: false,
        flag: false,
        ...
        existingPicture: '', //判断是否存在图片资源
        existingArticlePicture: '',   //推荐文章图片是否存在
        currentInfo: '',
        ArticleInfo: '',
        FiveValue: '',
        SixValue: ''
    ...
    methods: {
      ...
      //分类内容
      classification(prop) {
        this.existingPicture = prop.pictureResources[0]
        ...
        this.currentInfo = prop
      },
      //审核通过并分类
      determine() {
        this.classificationShow = false
        const firstTypeName = this.FirstClassificationValue
        const secondTypeName = this.SecondClassificationValue
        var secondId = this.options2.filter(item => {
        ...
        if(this.existingPicture) {
          this.$api.find.shenhe_pass(() => {
            ...
            checkType: 1,
            typeName: this.SecondClassificationValue,
            createTime: moment(new Date(this.currentInfo.createTime)).format("YYYY-MM-DD HH:mm:ss"),
            typeId: secondId[0].id,
            picture: this.currentInfo.pictureResources
          })
        } else {
          this.$api.find.shenhe_pass(() => {
            this.$notify({
      ...
      handleChange6() {
        this.search.classify = this.SixValue
      },
      chooseImg(index, address) {
        this.SelectedImageIndex = index
        this.pictureAddress = []
        this.pictureAddress.push(address)
      },
      //推荐文章分类
      ArticleClass(prop) {
        console.log(prop)
        ...
        this.ArticleInfo = prop
      },
      ArticleDetermine() {
        this.ArticleShow = false
        //用户自己有图片
        if(this.existingArticlePicture) {
          this.$api.find.updateCheckTypeAndType(() => {
            this.showList(this.page)
            ...
            checkType: 1,
            typeId: this.SecondArticleValue,
            picture: this.ArticleInfo.pictureResources
          })
        } else {
          // 官方自己添加图片
          this.$api.find.updateCheckTypeAndType(() => {
      ...
      checkClass() {
        this.centerDialogVisibleClass = true
      },
      //查询分类
      checkClassItem() {
